...
Deno.test("m.coat - Input Unset Cases", async (t) => {
	...
	await t.step("m.coat=[out=<in>] - unset input", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in>]])]'));
		assertEquals(html, '<div out=""></div>');
	});

	await t.step("m.coat=[out=<in?a>] - unset input, SET test", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in?a>]])]'));
		assertEquals(html, '<div out=""></div>');
	});

	await t.step("m.coat=[out=<in??a>] - unset input, SNE test", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in??a>]])]'));
		assertEquals(html, '<div out=""></div>');
	});

	await t.step("m.coat=[out=<in|a>] - unset input, UNS test", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in|a>]])]'));
		assertEquals(html, '<div out="a"></div>');
	});

	await t.step("m.coat=[out=<in||a>] - unset input, UOE test", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in||a>]])]'));
		assertEquals(html, '<div out="a"></div>');
	});

	await t.step("m.coat=[out=<in?a|b>] - unset input, SET test with toggles", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in?a|b>]])]'));
		assertEquals(html, '<div out="b"></div>');
	});

	await t.step("m.coat=[out=<in??a||b>] - unset input, SNE test with toggles", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in??a||b>]])]'));
		assertEquals(html, '<div out="b"></div>');
	});

	await t.step("m.coat=[out=<in?a|b|c|d>] - unset input, SET test with multiple toggles", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in?a|b|c|d>]])]'));
		assertEquals(html, '<div out="bd"></div>');
	});

	await t.step("m.coat=[out=<in??a|b|c|d>] - unset input, SNE test with multiple toggles", () => {
		const html = renderHTML(ps('[([h.div m.coat=[out=<in??a|b|c|d>]])]'));
		assertEquals(html, '<div out="bd"></div>');
	});
});
...
Deno.test("m.coat - Input Empty String Cases", async (t) => {
	...
	await t.step("m.coat=[out=<in>] in='' - empty input", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in>]]])]'));
		assertEquals(html, '<div out=""></div>');
	});

	await t.step("m.coat=[out=<in?a>] in='' - empty input, SET test", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in?a>]]])]'));
		assertEquals(html, '<div out="a"></div>');
	});

	await t.step("m.coat=[out=<in??a>] in='' - empty input, SNE test", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in??a>]]])]'));
		assertEquals(html, '<div out=""></div>');
	});

	await t.step("m.coat=[out=<in|a>] in='' - empty input, UNS test", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in|a>]]])]'));
		assertEquals(html, '<div out=""></div>');
	});

	await t.step("m.coat=[out=<in||a>] in='' - empty input, UOE test", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in||a>]]])]'));
		assertEquals(html, '<div out="a"></div>');
	});

	await t.step("m.coat=[out=<in?a|b>] in='' - empty input, SET with toggles", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in?a|b>]]])]'));
		assertEquals(html, '<div out="a"></div>');
	});

	await t.step("m.coat=[out=<in??a||b>] in='' - empty input, SNE with toggles", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in??a||b>]]])]'));
		assertEquals(html, '<div out="b"></div>');
	});

	await t.step("m.coat=[out=<in?a|b|c|d>] in='' - empty input, SET with multiple toggles", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in?a|b|c|d>]]])]'));
		assertEquals(html, '<div out="ac"></div>');
	});

	await t.step("m.coat=[out=<in??a|b|c|d>] in='' - empty input, SNE with multiple toggles", () => {
		const html = renderHTML(ps('[([src in="" [h.div m.coat=[out=<in??a|b|c|d>]]])]'));
		assertEquals(html, '<div out="bd"></div>');
	});
});
...
Deno.test("m.coat - Input Non-Empty String Cases", async (t) => {
	...
	await t.step("m.coat=[out=<in??a>] in=value - non-empty input, SNE test", () => {
		const html = renderHTML(ps('[([src in=value [h.div m.coat=[out=<in??a>]]])]'));
		assertEquals(html, '<div out="a"></div>', 'Should use "a" (in is SNE)');
	});

	await t.step("m.coat=[out=<in|a>] in=value - non-empty input, UNS fallback", () => {
		const html = renderHTML(ps('[([src in=value [h.div m.coat=[out=<in|a>]]])]'));
		assertEquals(html, '<div out="value"></div>', 'Should copy in value (in is SET and SNE)');
	});

	await t.step("m.coat=[out=<in||a>] in=value - non-empty input, UOE fallback", () => {
		const html = renderHTML(ps('[([src in=value [h.div m.coat=[out=<in||a>]]])]'));
		assertEquals(html, '<div out="value"></div>', 'Should copy in value (in is not UOE)');
	});

	await t.step("m.coat=[out=<in?a|b>] in=value - non-empty input, SET with toggles", () => {
	...
	await t.step("m.coat=[out=<in??a||b>] in=value - non-empty input, SNE with toggles", () => {
		const html = renderHTML(ps('[([src in=value [h.div m.coat=[out=<in??a||b>]]])]'));
		assertEquals(html, '<div out="a"></div>', 'Should use then "a" (in is SNE)');
	});

	await t.step("m.coat=[out=<in?a|b|c|d>] in=value - non-empty input, SET with multiple toggles", () => {
	...
	await t.step("m.coat=[out=<in??a|b|c|d>] in=value - non-empty input, SNE with multiple toggles", () => {
		const html = renderHTML(ps('[([src in=value [h.div m.coat=[out=<in??a|b|c|d>]]])]'));
...
Deno.test("m.coat - Multiple Attributes", async (t) => {
	await t.step("Multiple m.coat attributes with different expressions", () => {
	...
	await t.step("m.coat with regular attributes", () => {
...
Deno.test("m.coat - Edge Cases", async (t) => {
	await t.step("m.coat with numeric input", () => {
	...
	await t.step("m.coat with zero input, SET test", () => {
		const html = renderHTML(ps('[([src in=0 [h.div m.coat=[out=<in?zeroisset>]]])]'));
		assertEquals(html, '<div out="zeroisset"></div>');
	});

	await t.step("m.coat with zero input, SNE test", () => {
		const html = renderHTML(ps('[([src in=0 [h.div m.coat=[out=<in??zeroisset>]]])]'));
		assertEquals(html, '<div out="zeroisset"></div>');
	});

	await t.step("m.coat with boolean true input, SET with toggles", () => {
	...
	await t.step("m.coat with boolean false input, SET with toggles", () => {
	...
	await t.step("m.coat with special characters in values", () => {
		const html = renderHTML(ps('[([src in="<>&\\"" [h.div m.coat=[out=<in>]]])]'));
		assert(html.includes('out="&lt;&gt;&amp;&quot;"'), 'Should escape special characters');
	});

	await t.step("m.coat with nested element", () => {
...
Deno.test("m.coat - Complex Expression Chains", async (t) => {
	await t.step("Multiple outputs from same input", () => {
	...
	await t.step("Different inputs for different outputs", () => {
