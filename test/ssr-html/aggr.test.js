...
Deno.test('MWIDocument - getHTML with aggregate placeholder replacement', async (t) => {
	await t.step('(getHTML) - No aggregation: returns plain HTML', () => {
		...
		doc('append', ls(['list', '[([h.div hello])]']));
	...
	await t.step('(getHTML) - Replaces placeholder with aggregated content', () => {
		...
		doc('mapAggrBuffer', ls([, bufferKey]));
		...
		const aggrNode = doc('createNode', ls([, 'm.aggr']));
		aggrNode('setAttr', ['from', 'default']);
		doc('append', ls([, aggrNode]));
	...
	await t.step('(getHTML) - Multiple placeholders replaced correctly', () => {
		...
		doc('mapAggrBuffer', ls([, key1]));
		doc('mapAggrBuffer', ls([, key2]));
		...
		const fromNode1 = doc('createNode', ls([, 'm.aggr']));
		fromNode1('setAttr', ['from', 'buf1']);
		const fromNode2 = doc('createNode', ls([, 'm.aggr']));
		fromNode2('setAttr', ['from', 'buf2']);
		doc('append', ls([, fromNode1, , fromNode2]));
	...
	await t.step('(getHTML) - False-positive placeholders inside comments/scripts/styles are not replaced', () => {
		...
		commentNode('setAttr', ['t', '<{0}> is a fake placeholder']);
		doc('append', ls([, commentNode]));

		// 2. Create a real from node for the real buffer (real - comes second)
		const fromNode = doc('createNode', ls([, 'm.aggr']));
		fromNode('setAttr', ['from', 'real']);
		doc('append', ls([, fromNode]));
		...
		const scriptNode = doc('createNode', ls([, 'h.script']));
		scriptNode('setAttr', ['m.text', 'var x = "scr<{0}>ipt";']);
		doc('append', ls([, scriptNode]));
	...
	await t.step('(getHTML) - Same placeholder referenced multiple times: first renders, second warns and is empty, third+ are empty', () => {
		...
		const fromNode1 = doc('createNode', ls([, 'm.aggr']));
		fromNode1('setAttr', ['from', 'multi']);
		const fromNode2 = doc('createNode', ls([, 'm.aggr']));
		fromNode2('setAttr', ['from', 'multi']);
		const fromNode3 = doc('createNode', ls([, 'm.aggr']));
		fromNode3('setAttr', ['from', 'multi']);
		doc('append', ls([, fromNode1, , fromNode2, , fromNode3]));
