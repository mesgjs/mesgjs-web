...
Deno.test("MWICoreFrag (m.frg) - Spec Management", async (t) => {
	...
	await t.step("(getSubSpec) - Empty fragment returns empty NANOS", async () => {
		const fragNode = await doc.createNode('m.frg');
		const subSpec = fragNode('getSubSpec');
		assertEquals(subSpec.size, 0);
	});

	await t.step(".getSubSpec() - Empty fragment returns empty NANOS via JS", async () => {
		const fragNode = await doc.createNode('m.frg');
		const subSpec = fragNode.getSubSpec();
		assertEquals(subSpec.size, 0);
	});

	await t.step("(getSubSpec) - Fragment with children returns child specs", async () => {
		const fragNode = await doc.createNode('m.frg');
	...
	await t.step(".getSubSpec() - Fragment with children returns child specs via JS", async () => {
		const fragNode = await doc.createNode('m.frg');
		const textNode = await doc.createNode('m.t');
	...
	await t.step("(setSpec) - Set attributes from spec (async)", async () => {
		...
		await fragNode('setSpec', ls([, spec]));
	...
	await t.step(".setSpec() - Set children from spec via JS (async)", async () => {
		const fragNode = await doc.createNode('m.frg');
	...
	await t.step("(setSubSpec) - Set children with NANOS list (async)", async () => {
		const fragNode = await doc.createNode('m.frg');
		const subList = ps('[([m.t t="Sub 1"] [m.t t="Sub 2"])]');
		await fragNode('setSubSpec', ls([, subList]));
		const subSpec = fragNode('getSubSpec');
		assertEquals(subSpec.size, 2);
	...
	await t.step(".setSubSpec() - Set children with NANOS list via JS (async)", async () => {
		const fragNode = await doc.createNode('m.frg');
		const subList = ps('[([m.t t="JS Sub 1"] [m.t t="JS Sub 2"])]');
		await fragNode.setSubSpec([subList]);
		const subSpec = fragNode.getSubSpec();
		assertEquals(subSpec.size, 2);
	...
	await t.step("(setSubSpec) - Set children with spec parameter (async)", async () => {
		const fragNode = await doc.createNode('m.frg');
		const fullSpec = ps('[(m.frg [m.t t="Spec Child"])]');
		await fragNode('setSubSpec', ls(['spec', fullSpec]));
		const subSpec = fragNode('getSubSpec');
		assertEquals(subSpec.size, 1);
		assertEquals(subSpec.at([0, 't']), 'Spec Child');
	});

	await t.step(".setSubSpec() - Set children with spec parameter via JS (async)", async () => {
		const fragNode = await doc.createNode('m.frg');
		const fullSpec = ps('[(spec=[m.frg [m.t t="JS Spec Child"]])]');
		await fragNode.setSubSpec(fullSpec);
		const subSpec = fragNode.getSubSpec();
		assertEquals(subSpec.size, 1);
...
Deno.test("MWICoreFrag (m.frg) - Content Aggregation", async (t) => {
	...
	await t.step("(append) - Returns fragment for chaining", async () => {
		const fragNode = await doc.createNode('m.frg');
		const result = fragNode('append', ls([, 'text']));
		assertStrictEquals(result, fragNode);
	});

	await t.step(".append() - Returns fragment for chaining via JS", async () => {
		const fragNode = await doc.createNode('m.frg');
