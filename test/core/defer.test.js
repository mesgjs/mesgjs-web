...
Deno.test("MWICoreDefer (m.defer) - Basic Interface Tests", async (t) => {
	...
	await t.step("(type) - Get node type (should be h.slot after init)", async () => {
		// After @init, type changes from m.defer to h.slot
		assertEquals(deferNode('type'), 'h.slot');
	});

	await t.step(".type - Get node type via JS (should be h.slot after init)", async () => {
		assertEquals(deferNode.type, 'h.slot');
	});
...
Deno.test("MWICoreDefer (m.defer) - Defer Type Capture", async (t) => {
	await t.step("Captures original type in m.deferType attribute", async () => {
		const deferNode = doc.createNode('m.defer');
		// The original type 'm.defer' should be captured in m.deferType
		assertEquals(deferNode('getAttr', ls([, 'm.deferType'])), 'm.defer');
	});

	await t.step("Captures original type in data-mwi-defer attribute", async () => {
		const deferNode = doc.createNode('m.defer');
		assertEquals(deferNode('getAttr', ls([, 'data-mwi-defer'])), 'm.defer');
	});

	await t.step("Type changes to h.slot after init", async () => {
		const deferNode = doc.createNode('m.defer');
...
Deno.test("MWICoreDefer (m.defer) - Auto-ID Assignment", async (t) => {
	await t.step("Auto-assigns ID if not present", async () => {
		const deferNode = doc.createNode('m.defer');
	...
	await t.step("Preserves explicit ID if provided", async () => {
	...
	await t.step("Auto-assigned IDs are unique", async () => {
...
Deno.test("MWICoreDefer (m.defer) - Spec Management", async (t) => {
	await t.step("(getSpec) - Get spec with defer attributes", async () => {
		const deferNode = doc.createNode('m.defer');
	...
	await t.step(".getSpec() - Get spec with defer attributes via JS", async () => {
		const deferNode = doc.createNode('m.defer');
		deferNode.getAttr('m.id');
		const spec = deferNode.getSpec();
		assertEquals(spec.at(0), 'h.slot');
	...
	await t.step("(setSpec) - Set attributes from spec", async () => {
		const deferNode = doc.createNode('m.defer');
	...
	await t.step(".setSpec() - Set attributes from spec via JS", async () => {
		const deferNode = doc.createNode('m.defer');
...
Deno.test("MWICoreDefer (m.defer) - Sub-Spec Behavior", async (t) => {
	...
	await t.step("(setSubSpec) - Set sub-spec (getSubSpec still empty)", async () => {
		const deferNode = doc.createNode('m.defer');
		const subSpec = ls([, 'child1', , 'child2']);
		deferNode('setSubSpec', ls([, subSpec]));
		const resultSubSpec = deferNode('getSubSpec');
		assertEquals(resultSubSpec.size, 0);
	});

	await t.step(".setSubSpec() - Set sub-spec via JS (getSubSpec still empty)", async () => {
		const deferNode = doc.createNode('m.defer');
		const subSpec = ls([, 'child1', , 'child2']);
...
Deno.test("MWICoreDefer (m.defer) - Redispatch to MWIHTML", async (t) => {
	await t.step("Chains to MWIHTML interface", async () => {
		const deferNode = doc.createNode('m.defer');
...
Deno.test("MWICoreDefer (m.defer) - Real-World Creation via Document", async (t) => {
	await t.step("Document creates defer node for unloaded component", async () => {
	...
	await t.step("Document.from() creates defer nodes from specs", async () => {
	...
	await t.step("Mixed content with defer and loaded components", async () => {
...
Deno.test("MWICoreDefer (m.defer) - Complex Scenarios", async (t) => {
	await t.step("Defer node with multiple attributes", async () => {
	...
	await t.step("Multiple defer nodes with unique IDs", async () => {
		const defer1 = doc.createNode('m.defer');
		const defer2 = doc.createNode('m.defer');
	...
	await t.step("Defer node attribute modifications", async () => {
		const deferNode = doc.createNode('m.defer');
